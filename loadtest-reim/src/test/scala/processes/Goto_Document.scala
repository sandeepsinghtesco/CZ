...
object Goto_Document {
  val goto_Document =
    pause(1)                                                                               // User pause
    .exec(http("Sidebar_Document")                                                          // Sidebar document maintenance click
      .post("/ReimViewController/faces/Home")
        .headers(headers_2)
        ...
        .formParam("event", "pt1:showMenuPanelDynRegion:1:taskItr:1:l9")
        .formParam("event.pt1:showMenuPanelDynRegion:1:taskItr:1:l9", """<m xmlns="http://oracle.com/richClient/comm"><k v="type"><s>action</s></k></m>""")
        .formParam("oracle.adf.view.rich.PROCESS", "pt1:showMenuPanelDynRegion,pt1:showMenuPanelDynRegion:1:taskItr:1:l9")
      .check(bodyString.saveAs("Answer"))
      .check(regex("(?<=ViewState\" value=\").*?(?=\")").optional.saveAs("ViewState"))
    )
    .pause(1)                                                                               // User pause
    .exec(http("Sidebar_Search")                                                            //Sidebar document search click
